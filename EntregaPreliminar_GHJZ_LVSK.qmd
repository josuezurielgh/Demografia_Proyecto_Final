...
format: 
  pdf:
    papersize: letter
    geometry:
      - top=3cm       # Espacio para que el encabezado no choque con el texto
      - bottom=2.5cm
      - left=2.5cm
      - right=2.5cm
    include-in-header:
      text: 
        ...
        \renewcommand{\headrulewidth}{0.4pt} % Línea debajo del encabezado (opcional)
        \usepackage{xcolor} 
        \usepackage{titlesec}
        \usepackage{eso-pic}
        ...
        \renewcommand{\@maketitle}{
          \begin{center}
            \vspace*{1cm}
            {\Huge\sffamily\bfseries\color{->} \@title \par}
            \vspace*{0.5cm}
...
# Cargamos los datos
datos_edr <- read.csv("EDR_Chihuahua_2008-2023.csv")
...
# Leemos el archivo
datos <- read.csv("Muertes_por_Homicidia_Chihuahua.csv")
# Graficamos
...
datos_edad <- read.csv("Muertes_Homicidios_por_edad_Chihuahua.csv")
...
datos <- read.csv("Principales_Enfermedades.csv", check.names = FALSE)
...
# Leemos el documento
datos_enfermedad <- read.csv("Muertes_por_Enfermedad_Chihuahua.csv")
...
ggplot(datos_enfermedad, aes(x = year, y = total, color = sexo, group = sexo)) +
  ...
  labs(
    ...
    caption = paste0 ("Fuente: Elaboración propia\n", 
                     "El procesamiento de los datos se puede ver a detalle en el documento Datos.csv"),
    x = "Año",
...
datos_enfermedad_edad <- read.csv("Muertes_por_Enfermedad_edad_Chihuahua.csv")
...
datos_enfermedad_edad %>% 
  filter(sexo == "Hombres") %>% 
  ggplot(aes(x = edad, y = total)) +
  geom_col(fill = "#1f77b4", color = "white", alpha = 0.85) +
  labs(
    x = "Grupos de Edad", 
    y = "Total de Defunciones"
  ) +
...
# \color{->} 2. Tablas de vida ![](images/formula.png){height="1.5em"}
...
## \color{->} 2.1. Procesamiento de los datos

Un censo es un recuento detallado de todos los individuos que componen una población, por lo que, es de vital importante para múltiples recursos demográficos; en particular, para una *Tabla de Vida*. En México, contamos con un organismo que dentro de otras funciones, se encarga de la recopilación y publicación de los datos censales, dicho esto, nos ayudaremos de los censos poblacionales hechos por el INEGI en 2010 y 2020, y los consultaremos desde su sitio oficial.
...
## \color{->} 2.2 Fórmulas para la Tabla de Vida Abreviada
...
## \color{->} 2.3 Diagrama de flujo de la construcción de la Tabla de vida
...
![](diagrama_tabla_vida.png){fig-align="center"}

## \color{->} 2.4 Tablas de vida en R
...
library(data.table)
mh_x <- fread("Tasas_Mortalidad_Chihuahua.csv")[Sexo=="m"]
mm_x <- fread("Tasas_Mortalidad_Chihuahua.csv")[Sexo=="f"]
...
## \color{->} 2.5 Tabla de esperanza de vida al nacer
...
# setwd("C:/Users/DELL/Documents/Demografia 2026-1/Proyecto_Chihuahua_2026_1")
